...
BANK_STATEMENT_VALUES = 'Unnamed: 3'
BANK_STATEMENT_INFO   = 'Unnamed: 1'
...
categories = {"FPHSpecifiers":0, "GroceriesSpecifiers":0, "HomeRenoSpecifiers":0,
...
for category_name in categories.keys():
    ...
    purchases = df_copy.loc[df_copy['Unnamed: 1'].str.extract(f'({pattern})', expand=False).notna()]
    ...
    for purchase in purchase_list:
        purchase_data = [{"Category": category_name, "Description": purchase[0], "Amount": purchase[1]} for purchase in purchase_list]
        purchase_df = pd.concat([purchase_df, pd.DataFrame(purchase_data)], ignore_index=True)
